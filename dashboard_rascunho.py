...
<style>
  ...
  .inst-title { font-size: 1rem; font-weight: 600; color: #0f172a; margin: 0; line-height: 1.3; }
  .inst-sub   { font-size: 0.73rem; color: #64748b; margin: 0; }
...
<div class="inst-header">
  <span class="inst-badge">⚖ TJMG · COPAT</span>
  <div>
    <p class="inst-title">Controle de Estoque — Entrada de Notas Fiscais</p>
    <p class="inst-sub">Coordenadoria de Patrimônio · Tribunal de Justiça de Minas Gerais</p>
  </div>
</div>
""", unsafe_allow_html=True)
...
with st.spinner("Consultando BigQuery..."):
    try:
        if client is None:
            raise ConnectionError("Sem credenciais")
...
if modo_demo:
    st.info("Modo demonstração — dados sintéticos (BigQuery indisponível localmente).", icon="ℹ")
...
with col1:
    if "Data_Contabilizacao" in df_notas.columns:
        ...
        st.bar_chart(by_mes.set_index("Mês")["NFs"], height=260, use_container_width=True)
    else:
...
with col2:
    if "CATMAS" in df_itens_f.columns and "Quantidade" in df_itens_f.columns:
        ...
        st.bar_chart(top_cat.set_index("CATMAS")["Quantidade"], height=260, use_container_width=True)
    else:
...
with tab1:
    ...
    st.dataframe(
        df_sorted, use_container_width=True, height=420, hide_index=True,
        column_config={
...
with tab2:
    ...
    st.dataframe(
        df_show, use_container_width=True, height=420, hide_index=True,
        column_config={
            "NF":        st.column_config.TextColumn("Nº NF",               width="small"),
            "Serie":     st.column_config.TextColumn("Série",               width="small"),
